from PySide6.QtWidgets import QApplication, QMainWindow, QTableWidgetItem, QFileDialog, QLineEdit, QTableWidget, QApplication
...
class MainWindow(QMainWindow):
        ...
        try:
            ...
            if event.key() == Qt.Key.Key_V and (event.modifiers() & Qt.KeyboardModifier.ControlModifier):
                ...
                if selection:
                    ...
                    if self.ui.TableServices.rowCount() < row_anchor + len(rows) - 1:
                        self.ui.TableServices.setRowCount(
                            row_anchor + len(rows) - 1)
            ...
            if (event.key() == Qt.Key.Key_C or event.key() == Qt.Key.Key_X) \
                ...
                for cell in copied_cell:
                    # Get each cell text
                    cell_item = self.ui.TableServices.item(
    ...
    def procesar_oc(self, filepath):
        ...
        detailed_purchase_orders = detailed_purchase_orders.reset_index(
            drop=True)
        print(detailed_purchase_orders)
        ...
        print(simplified_purchase_orders)
        ...
        for inx in range(len(purchase_order_indexs)-1):
            print(purchase_order_indexs[inx],
                  " - ", purchase_order_indexs[inx+1])
    ...
    def process_files(self):
        ...
        simplified_service_orders = simplified_service_orders.reset_index(
            drop=True)
