...
products_workbook = pd.read_excel('Productos.xlsx')

products_cost_df = products_workbook[['SKU', 'Costo']]
print(products_cost_df)
...
for inx in range(len(servces_index)-1):
    ...
    items_with_costs = itemsDF.merge(products_cost_df, how='left', on='SKU')
    items_cost = servces_index[inx], list(items_with_costs[[
...
os.insert(5, "Costo total", total_cost_col)
os.insert(6, "Garantía?", garantia_col)
